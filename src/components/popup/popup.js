      ...
      if (tabs[0]?.id) {
        ...
        chrome.tabs.sendMessage(tab.id, { action: 'getTheme' }, response => {
          if (chrome.runtime.lastError) {
            console.log(
              'Popup: Content script not available, using direct detection'
            );
...
function executeThemeDetectionScript(tabId) {
  ...
  if (chrome.scripting) {
      ...
      .executeScript({
        ...
        func: () => {
          ...
          try {
            ...
            console.log(
              'Direct script: localStorage colorTheme =',
              roll20Theme
            );

            if (roll20Theme === 'light') {
              console.log(
                'Direct script: Detected light theme from localStorage'
              );
              return 'light';
            } else if (roll20Theme === 'dark') {
              console.log(
                'Direct script: Detected dark theme from localStorage'
              );
              return 'dark';
            }
          } catch (e) {
          ...
          if (
            ...
            console.log(
              'Direct script: Detected light theme from roll20 classes'
            );
            return 'light';
          }
    ...
    console.log(
      'Popup: chrome.scripting not available, defaulting to dark theme'
    );
    applyTheme('dark');
  }
}
